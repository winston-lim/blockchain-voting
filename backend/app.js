...
// Example POST route for signature blinding
app.post('/', async (req, res) => {
  ...
  const r = '1234567890abcdef'; // Random blinding factor (hex string)
  console.log('Blinding message...');
  const blindedMessage = blindMessage(message, N, E, r);
  console.log('Blinded Message:', blindedMessage);

  // Server-side signing
  console.log('Signing blinded message...');
  const signedMessage = signBlindedMessage(blindedMessage, privateKey);
  console.log('Signed Blinded Message:', signedMessage);

  // Client-side unblinding
  console.log('Unblinding signed message...');
  const unblindedSignature = unblindSignature(signedMessage, r, N);
  console.log('Unblinded Signature:', unblindedSignature);

  // Client-side verification
  const isClientVerified = verifySignature(
    message,
    unblindedSignature,
    publicKey
  );
  if (!isClientVerified) {
    return res.status(500).json('Client: Invalid signature');
  }

  // Server-side verification
  const isServerVerified = verifySignature(
    message,
    unblindedSignature,
    publicKey
  );
  if (isServerVerified) {
    ...
    return res.status(500).json('Server: Invalid signature');
  }
});
