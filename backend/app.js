...
app.post('/blind', async (req, res) => {
    ...
    const blindingFactor = '0x' + crypto.randomBytes(32).toString('hex');
    const blindedHash = '0x' + (BigInt(tokenHash) ^ BigInt(blindingFactor)).toString(16);

    // Admin signs the blinded tokenHash
    const [signedBlindedHash, walletAddress] = await signMessage(blindedHash);

    const signedHash = '0x' + (BigInt(signedBlindedHash) ^ BigInt(blindingFactor)).toString(16);
    const signerAddress = ethers.verifyMessage(ethers.getBytes(tokenHash), signedHash);
    console.log({
        token,
        tokenHash,
        blindingFactor,
    ...
    // votingManager.methods.castVote(0, vote, token, signedTokenHash)
});
